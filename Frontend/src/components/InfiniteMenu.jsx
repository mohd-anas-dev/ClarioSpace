...
class InfiniteGridMenu {
  ...
  run(time = 0) {
    if (!this.gl || !this.discProgram) {
      console.error('Cannot run: missing WebGL context or shader program');
      return;
    }
  ...
  #init(onInit) {
    ...
    if (!gl) {
      ...
      fallbackDiv.innerHTML = 'WebGL 2 is required but not supported in your browser.';
      fallbackDiv.style.cssText = 'color: white; padding: 20px; text-align: center; background: #333;';
      this.canvas.parentNode.appendChild(fallbackDiv);
      if (onInit) onInit(null);
      return;
    }
  ...
  #initTexture() {
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      // Draw a colorful placeholder
      const hue = (i * 137.508) % 360; // Golden angle approximation
      ctx.fillStyle = `hsl(${hue}, 70%, 50%)`;
      ctx.fillRect(x + 50, y + 50, cellSize - 100, cellSize - 100);
    ...
    // Then try to load actual images
    Promise.all(
      this.items.map(
        (item, index) =>
          new Promise(resolve => {
            ...
            img.onerror = (e) => {
              console.error(`Failed to load image ${index + 1}:`, item.image, e);
      ...
      results.forEach(({ img, index }) => {
        ...
        if (img) {
          ...
          // Create clipping path for the cell area
          ctx.beginPath();
          ...
          // Keep the placeholder for failed images with shadow
          ctx.save();
          ctx.shadowColor = 'black';
          ctx.shadowBlur = 4;
          ctx.shadowOffsetX = 0;
  ...
  #animate(deltaTime) {
    const gl = this.gl;
    if (!this.control) {
      console.error('Control object not initialized');
      return;
    }
  ...
  #render() {
    ...
    if (!gl || !this.discProgram) {
      console.error('Cannot render: missing WebGL context or shader program');
      return;
    }
    ...
    gl.clearColor(243/255, 245/255, 247/255, 1); // Light gray background (243, 245, 247) //! WebGL Background color
...
export default function InfiniteMenu({ items = [] }) {
        ...
        sketch = new InfiniteGridMenu(
          ...
          sk => {
            if (sk && sk.gl) {
              ...
              console.error('InfiniteGridMenu initialization failed');
              setError('Failed to initialize WebGL graphics');
            }
          }
        );
      } catch (err) {
        console.error('Error creating InfiniteGridMenu:', err);
        setError(`Graphics initialization failed: ${err.message}`);
      }
    }
  ...
  if (error) {
    return (
      <div className="relative w-full h-full flex items-center justify-center bg-gray-800 text-white">
        <div className="text-center">
          <h3 className="text-xl mb-2">Graphics Error</h3>
